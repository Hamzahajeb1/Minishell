...
{
	if (!ft_strcmp(sh->cmd->argv[0], "pwd"))
		ft_pwd(sh);
	else if (!ft_strcmp(sh->cmd->argv[0], "echo"))
		ft_echo(sh);
	else if (!ft_strcmp(sh->cmd->argv[0], "env"))
		ft_env(sh);
	else if (!ft_strcmp(sh->cmd->argv[0], "unset"))
		ft_unset(sh);
	else if (!ft_strcmp(sh->cmd->argv[0], "export"))
		ft_export(sh);
	else if (!ft_strcmp(sh->cmd->argv[0], "cd"))
		ft_cd(sh);
	else if (!ft_strcmp(sh->cmd->argv[0], "exit"))
		ft_exit(sh->cmd->argv);
...
void    execute_with_execve(t_data *sh, char *path)
{
    ...
    execve(path, sh->cmd->argv, sh->env);
    perror("minishell");
    exit(127);
}
...
char *find_executable_path(t_data *sh)
{
    ...
    path = path_search(sh->cmd->argv[0], sh->env);
    if (!path)
    {
        handle_exec_error(sh->cmd->argv[0]); //to do
        return (NULL);
    }
    return (path);
}
